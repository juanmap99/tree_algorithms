...
export class StandardTree implements Tree{
    ...
    async inOrderTraversal(curNode: TreeNode, res : number[], prevColors : string[]){
        prevColors.push(curNode.getColor());
        curNode.setColor("#FBC68C");
        await this.delayByRunSpeed();
        if(curNode.lChild){
            await this.inOrderTraversal(curNode.lChild,res,prevColors)
        }
        curNode.setColor("#F39530");
        res.push(curNode.valor);
        await this.delayByRunSpeed();
        if(curNode.rChild){
            await this.inOrderTraversal(curNode.rChild,res,prevColors)
        }
    }

    async preOrderTraversal(curNode: TreeNode, res : number[], prevColors : string[]){
        prevColors.push(curNode.getColor());
        curNode.setColor("#F39530");
        res.push(curNode.valor);
        await this.delayByRunSpeed();
        if(curNode.lChild){
            await this.preOrderTraversal(curNode.lChild,res,prevColors)
        }
        if(curNode.rChild){
            await this.preOrderTraversal(curNode.rChild,res,prevColors)
        }
    }

    async postOrderTraversal(curNode: TreeNode, res : number[], prevColors : string[]){
        prevColors.push(curNode.getColor());
        curNode.setColor("#FBC68C");
        await this.delayByRunSpeed();
        if(curNode.lChild){
            await this.postOrderTraversal(curNode.lChild,res,prevColors)
        }
        if(curNode.rChild){
            await this.postOrderTraversal(curNode.rChild,res,prevColors)
        }
        curNode.setColor("#F39530");
        res.push(curNode.valor);
        await this.delayByRunSpeed();
    ...
    async traverse(traverseType : string) : Promise<number[]>{
        let arrayResultate : number[] = []
        let previousColors : string[] = []
        if(traverseType == "InOrder"){
            await this.inOrderTraversal(this.root,arrayResultate,previousColors);
        }
        else if(traverseType == "PreOrder"){
            await this.preOrderTraversal(this.root,arrayResultate,previousColors);
        }
        else if(traverseType == "PostOrder"){
            await this.postOrderTraversal(this.root,arrayResultate,previousColors);
        }
        this.returnToPreviousColors(this.root,previousColors,0);
        await this.delayByRunSpeed();
        return arrayResultate;
    }
    ...
    returnToPreviousColors(curNode : TreeNode, previousColors : string[], index: number){
        curNode.setColor(previousColors[index])
        let nextIndex : number = index+1;
        if(curNode.lChild){
            nextIndex = this.returnToPreviousColors(curNode.lChild,previousColors,nextIndex)
        }
        if(curNode.rChild){
            nextIndex = this.returnToPreviousColors(curNode.rChild,previousColors,nextIndex)
        }
        return nextIndex;
    }
