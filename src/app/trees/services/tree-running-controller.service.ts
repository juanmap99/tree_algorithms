  ...
  async runMethod(metodo : string, value : number = 0){
    ...
    switch(metodo){
      ...
      case("FindMax"):
        ...
        break; 
      case("InOrder"):
        let arrayInOrder : number[] =  await this.arbol.traverse("InOrder");
        break; 
      case("PreOrder"):
        let arrayPreOrder : number[] = await this.arbol.traverse("PreOrder");
        break; 
      case("PostOrder"):
        let arrayPostOrder : number[] = await this.arbol.traverse("PostOrder");
        break; 
    }
    this.running = false;
    this.updateRunningObs();
